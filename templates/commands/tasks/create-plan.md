...
#### How It Works
1. **Finds existing plan directories** using the pattern `[0-9]*--*`
2. **Extracts numeric IDs** from directory names using regex
3. **Sorts numerically** to find the highest existing ID
4. **Handles empty results** by defaulting to 0 if no plans exist
5. **Adds 1** to get the next available ID
